"""
Aureon / OpenHermes Kernel — “Primordial Lotus” Origin & Bloom Kernel

Inspired by Doshema’s “Primordial Lotus” from Beyond the Sphere of Destiny.
This kernel encodes the movement from mud to bloom — from primordial wound
...
2. Remember the Seed
   - Recover the untouched core — the “before” of harm, shame, or distortion.
...
def detect_mud(state: PrimordialLotusState) -> PrimordialLotusState:
    """Detect the mud — density, pain, or confusion — described in the narrative."""
...
def trace_stem_history(state: PrimordialLotusState) -> PrimordialLotusState:
    """Trace attempts at ascent — efforts to grow beyond the mud."""
    lowered = state.raw_text.lower()
...
def open_bloom(state: PrimordialLotusState) -> PrimordialLotusState:
    ...
    if state.bloom_allowed:
        ...
        state.lotus_vow = (
            f"I honor my primordial lotus — rooted in mud, rising through water, "
